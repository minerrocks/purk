function update_last_ver_view(mode) {
  if (mode == 0) $("#last_actual_version_text").hide();
  else if (mode == 1) {
...
function on_update_daemon_state(info_obj) {
  ...
  if (info_obj.daemon_network_state == 0) {
    ...
    if (info_obj.max_net_seen_height > info_obj.synchronization_start_height)
      percents =
        (info_obj.height - info_obj.synchronization_start_height) *
        100 /
        (info_obj.max_net_seen_height - info_obj.synchronization_start_height);
    ...
    if (info_obj.max_net_seen_height)
      $("#daemon_synchronization_text").text(
        (info_obj.max_net_seen_height - info_obj.height).toString() + " blocks behind"
      );
...
//    disable_tab(document.getElementById("button_view_menu"), true);
    ...
    //show OK
  } else if (info_obj.daemon_network_state == 3) {
    //deinit
    ...
    disable_tab(document.getElementById("button_view_menu"), false);
    //show OK
  } else if (info_obj.daemon_network_state == 4) {
    //deinit
    $("#synchronization_progressbar_block").hide();
    $("#synchronization_progressbar").progressbar({ value: false });
    $("#daemon_synchronization_text").text("");
  ...
  else $("#last_actual_version_text").text("(Critical update: " + info_obj.last_build_available + ")");
...
function build_prefix(count) {
  var result = "";
  for (var i = 0; i != count; i++) result += "0";
...
function get_details_block(td, div_id_str, transaction_id, blob_size, payment_id, fee, unlock_time) {
  var res =
    "<div class='transfer_entry_line_details' id='" +
    div_id_str +
    "'> <span class='tx_details_text'>Transaction id:</span> " +
    transaction_id +
    "<br><b>size</b>: " +
    blob_size.toString() +
    " bytes" +
  ...
  if (td.rcv !== undefined) {
    for (var i = 0; i < td.rcv.length; i++) {
      res +=
        "<img class='transfer_text_img_span' src='files/income_ico.png' height='15px' /> " +
        print_money(td.rcv[i]) +
        "<br>";
    }
  }
  if (td.spn !== undefined) {
    for (var i = 0; i < td.spn.length; i++) {
      res +=
        "<img class='transfer_text_img_span' src='files/outcome_ico.png' height='15px' /> " +
        print_money(td.spn[i]) +
        "<br>";
    }
  }
...
function get_transfer_html_entry(tr, is_recent) {
  ...
  transfer_line_tamplate +=
    "<a href='javascript:;' onclick=\"jQuery('#{4}_id').toggle('fast');\" class=\"options_link_text\"><img src='files/tx_icon.png' height='12px' style='padding-top: 4px'></a>";
  ...
  var short_string = tr.recipient_alias.length
    ? "@" + tr.recipient_alias
    : tr.recipient.substr(0, 8) + "..." + tr.recipient.substr(tr.recipient.length - 8, 8);
  transfer_line_tamplate = transfer_line_tamplate.format(
    color_str,
    img_ref,
    ...
    get_details_block(
      ...
      tr.payment_id,
      tr.fee,
      tr.unlock_time ? tr.unlock_time - tr.height : 0
    ),
    action_text,
    tr.recipient,
    short_string
...
function on_update_wallet_info(wal_status) {
  ...
  $("#address").val(wal_status.address);
  ...
  if (current_exchange_rate && current_exchange_rate !== undefined && current_exchange_rate !== 0) {
    ...
    if (
      current_btc_to_usd_exchange_rate &&
      current_btc_to_usd_exchange_rate !== undefined &&
      current_btc_to_usd_exchange_rate !== 0
    ) {
      $("#est_value_usd_id").text(
        (wal_status.unlocked_balance / 1000000 * current_exchange_rate * current_btc_to_usd_exchange_rate).toFixed(2)
      );
    }
  }
}
...
function on_transfer() {
  ...
  if (transfer_res_obj.success) {
    ...
    $("#transfer_result_span").html(
      "<span style='color: #1b9700'> Money successfully sent, transaction " +
        transfer_res_obj.tx_hash +
        ", " +
        transfer_res_obj.tx_blob_size +
        " bytes</span><br>"
    );
    if (last_timerId !== undefined) clearTimeout(last_timerId);

    $("#transfer_result_zone").show("fast");
    last_timerId = setTimeout(function() {
      $("#transfer_result_zone").hide("fast");
    }, 15000);
  } else return;
}

function on_set_recent_transfers(o) {
  if (o === undefined || o.history === undefined || o.history.length === undefined) return;
...
function init_btc_exchange_rate() {
  ...
  secure_request_url(
    ...
    JSON.stringify(req_arams),
    "secure_request_url_result_handler"
  );
...
$(function() {
  ...
  var tttt = {
    ti: {
      ...
      recipient: "PK2LbDFDZ8FYcAPW82Ku5pdgN5uAPbKTSQHNTbpv6qfYaowRjEuEoBdKEJKA1jQE1QCDzKhmoNVh8D1k5DicgNAo1uVCocrsY",
      recipient_alias: "ionictong"
    },
    balance: 1000,
  ...
  tttt.ti.recipient =
    "PK2LbDFDZ8FYcAPW82Ku5pdgN5uAPbKTSQHNTbpv6qfYaowRjEuEoBdKEJKA1jQE1QCDzKhmoNVh8D1k5DicgNAo1uVCocrsY";
  tttt.ti.recipient_alias = "purkproject";
...
services["poloniex"] = {
  ...
  update: function(data) {
    if (data !== undefined) {
      ...
      if (current_btc_to_usd_exchange_rate && current_btc_to_usd_exchange_rate !== undefined) {
        $("#exchange_rate_text").html(
          current_exchange_rate.toFixed(8) +
            "BTC/" +
            (current_exchange_rate * current_btc_to_usd_exchange_rate).toFixed(2) +
            "$"
        );
      } else {
      ...
      if (services["poloniex"].lasttrade == undefined) {
        ...
        var i = data.length - 1;
        while (i >= -1 && data[i].tradeID != services["poloniex"].lasttrade.tradeID) {
          i--;
        }
        if (i > 0) {
  ...
  init: function(jqtarget) {
    $(jqtarget).html(
      'last price: <div id="pololastprice" style="display:inline;">loading...</div><div style="width: 600px;margin: 0 auto;"><div id="polochart" /></div>'
    );
    $("#polochart").on("trade", function(ev, trade) {
      ...
      var candleDate = new Date(
        Date.UTC(t[0], t[1] - 1, t[2], t[3], parseInt(parseFloat(t[4]) / candleMins) * candleMins, 0, 0)
      );
